...
STYLES = {
    'keyword': format_('orange', 'bold'),
    'operator': format_('brown'),
    'brace': format_('darkGray'),
    'defclass': format_('magenta'),
    'string': format_('lightBlue'),
    'string2': format_('darkGreen'),
    'comment': format_('darkGreen', 'italic'),
    'self': format_('magenta', 'italic'),
    'numbers': format_('lightGreen'),
}
...
class MelHighlighter(QSyntaxHighlighter):
    ...
    # Mel keywords
    keywords = [
        'and', 'as', 'case', 'catch', 'continue', 'do', 'else', 'exit', 'false', 'for', 'from', 'if', 'in', 'local',
        'not', 'of', 'off', 'on', 'or', 'random', 'return', 'then', 'throw', 'to', 'true', 'try', 'when', 'where',
        'while', 'with', 'vector',
    ]

    keywords2 = ['`', 'string', 'float', 'int', 'array', '-']
    ...
    def __init__(self, document):
        QSyntaxHighlighter.__init__(self, document)
        self.comments = (QRegExp(r"/\*"), QRegExp(r"\*/"), 1, STYLES['string2'])
        rules = []
    ...
    def highlightBlock(self, text):
        ...
        # Do multi-line strings
        self.match_multiline(text, *self.comments)
